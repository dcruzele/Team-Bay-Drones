...
if reprocess_SNR_flag == 1
    ...
    for i1 = 1:length(STCFIT.out_fit)
         ...
         [STCFIT.out_fit(i1).SG_fit.Ux_fit, STCFIT.out_fit(i1).SG_fit.Uy_fit, STCFIT.out_fit(i1).SG_fit.SNR_density_max, lin_ind] = retrieve_best_fit_from_SNR_2D(STCFIT.out_fit(i1).SG_fit.SNR_density_2D, STCFIT.out_fit(i1).SG_fit.Ux_2D, STCFIT.out_fit(i1).SG_fit.Uy_2D,0,0);
...
Camera_currents = struct('x',x,'y',y,'Ux',Ux,'Uy',Uy,'SNR',SNR, 'SNR_density',SNR_density,'time_stamp',STCFIT.Generic.time_stamp, 'currentdir_flag',currentdir_flag);
...
try
    ...
    UTM_currents = struct('x',x_utm,'y',y_utm,'Ux',Ux_utm,'Uy',Uy_utm,'SNR',SNR, 'SNR_density',SNR_density,'time_stamp',STCFIT.Generic.time_stamp, 'utmzone',utmzone,'uav_x_utm',uav_x_utm,'uav_y_utm',uav_y_utm, 'currentdir_flag',currentdir_flag);
